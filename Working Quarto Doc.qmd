...
Carbon pricing changed the rate of carbon emission decrease per year faster than without. H0: Slope(s) before and after the breakpoint equal 0 HA: Slope(s) Slopes after the breakpoint are less than 0.
...
To create a simulated dataset of emissions, we utilized the 
...
Let's create a dataframe where we utilize our segmented model to mutate the original emissions dataframe. We will also create a prediction grid to show a ribbon around the summary model.  
...
With this prediction dataframe, we have the 
